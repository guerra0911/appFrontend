...
const FloatingCard = ({ children, scaleValue }) => {
  return (
...
const ChallengeCardAnimated = ({ challenge, onLikeDislikeUpdate }) => {
  ...
  const challengerScale = useRef(new Animated.Value(1)).current;
  const originalScale = useRef(new Animated.Value(1)).current;

  useEffect(() => {
    const animate = () => {
      Animated.sequence([
        Animated.timing(challengerScale, {
          toValue: 1.1, // Increase this value for larger expansion
          duration: 3000, // Adjust the duration for how long it takes to expand
          useNativeDriver: true,
        }),
        Animated.timing(challengerScale, {
          toValue: 1, // This is the original size
          duration: 3000, // Adjust the duration for how long it takes to shrink back
          useNativeDriver: true,
        }),
        Animated.delay(3000),
    ...
    animate();
  }, [challengerScale, originalScale]);

  return (
    <View style={styles.challengeCard}>
      <View style={styles.postContainer}>
        <FloatingCard scaleValue={challengerScale}>
          <View style={styles.postCard}>
            ...
            >
              <Text style={styles.modalTitle}>
                {challengerPostActions.likeDislikeModalTitle}
              </Text>
              <ScrollView>
                {(challengerPostActions.likeDislikeModalTitle === "Liked By"
                  ? challengerPostActions.likedBy
                  : challengerPostActions.dislikedBy
        ...
        <FloatingCard scaleValue={originalScale}>
          <View style={styles.postCard}>
            <View style={styles.header}>
              <TouchableOpacity
            ...
            >
              <Text style={styles.modalTitle}>
                {originalPostActions.likeDislikeModalTitle}
              </Text>
              <ScrollView>
                {(originalPostActions.likeDislikeModalTitle === "Liked By"
                  ? originalPostActions.likedBy
                  : originalPostActions.dislikedBy
                ).map((user, index, array) => (
                  <View key={user.id}>
                    <TouchableOpacity
                      key={user.id}
...
export default ChallengeCardAnimated;
